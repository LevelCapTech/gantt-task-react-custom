# 1. ゴール / 非ゴール
- ゴール: Task データモデルに工程・担当者・予定/実績・状況の 7 項目を追加する設計を確定し、型定義・サンプルデータ・UI/状態管理の反映方針を明文化する。
- 非ゴール: コード実装や API/DB 連携、i18n 対応、外部ユーザー情報との同期。

# 2. スコープと変更対象
- 変更ファイル（新規/修正/削除）: `src/types/public-types.ts`（Task 型拡張と選択肢定義の追加）、`src/components/gantt/task-gantt-content.tsx` `src/components/gantt/gantt.tsx` `src/components/task-item/task-item.tsx` `src/components/task-list/task-list.tsx` `src/components/other/tooltip.tsx`（新フィールドの受け渡し/表示）、`example/src/helper.tsx` などサンプルデータ生成、必要に応じて選択肢定数の切り出しファイルを新設。
...
# 3. 設計方針
- 責務分離 / データフロー: `Task` に `process`/`status` のユニオン型（選択肢定数と併用）と予定/実績系フィールドを追加し、`Gantt` → `TaskGanttContent` → `TaskItem`/`TaskList`/`Tooltip` まで同型で透過的に渡す。リスト側で `process`/`status` を選択式入力、`assignee` を表示、`plannedStart`/`plannedEnd` を日付表示、`plannedEffort`/`actualEffort` はツールチップ/詳細表示に載せる。
- エッジケース / 例外系 / リトライ方針: 不正な `process`/`status` は定数一覧に含まれるか検証し、未定義・不正時はデフォルト値へフォールバック。`planned*`/`*Effort` 未設定は空表示、数値は 0 以上のみ許容とし負数は 0 or undefined に丸める設計。既存タスクに新規フィールドがなくてもクラッシュしないよう正常系優先で扱う。
