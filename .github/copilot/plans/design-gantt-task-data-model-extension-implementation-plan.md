# 1. 機能要件 / 非機能要件
- 機能要件:
  - Task データモデルに 7 項目（`process`/`assignee`/`plannedStart`/`plannedEnd`/`plannedEffort`/`actualEffort`/`status`）を追加し、UI（リスト・バー・ツールチップ）に表示/編集を反映する。
  - `process`/`status` の選択肢を定数/enumで固定し、UI から選択式で更新できる。
  - 工数単位を MH/MD/MM で切り替え可能とし、UI 表示と計算が選択単位に追従する。
  - 日付はリスト/ツールチップ両方に `yyyy-mm-dd` 形式で表示し、React のカレンダーコントロールで入力/バリデーションする。
  - 表示列は `visibleFields` 設定でオン/オフ可能とし、折りたたみ行でも新列が同期して表示される。
- 非機能要件:
  - バッジ色や列幅などは CSS 変数/テーマ props で変更可能にする（デフォルト色は本ドキュメントで定義）。
  - 既存互換性を維持し、新フィールド未指定時は空欄表示とする。
  - ログには個人情報/機微情報を出さず、ID/enum 値のみ記録する。
...
- バッジ色と単位切替:
  - `status` バッジのデフォルト色（例）: 未着手=#9e9e9e, 進行中=#2196f3, 完了=#4caf50, 保留=#ff9800。CSS 変数またはテーマ props で上書き可能にする。
  - 工数単位は MH/MD/MM を選択式で切替可能とし、表示/集計は選択単位に従う（内部値は h を基準に保持し換算する）。
- エッジケース / 例外系 / リトライ方針:
  ...
  - `planned*`/`*Effort` 未設定は空表示（デフォルト空欄）とし、工数は h 単位を基準に 0 以上の number のみ許容する（表示は選択単位 MH/MD/MM へ換算）。
  - 負数の工数は未設定（undefined）として扱い、計算に含めない。
  - 既存タスクに新規フィールドがなくてもクラッシュしないよう正常系優先で扱う。
- 日付バリデーション/表示: React のカレンダーコントロールで `yyyy-mm-dd` 書式入力を強制し、リスト/ツールチップの両方で同書式表示。未入力時は空欄表示。
