...
- エッジケース / 例外系 / リトライ方針:
  - 不正な `process`/`status` は選択肢定数に含まれない値（`undefined`/`null`/空文字/大小文字揺れ/異なる文字列）として扱い、デフォルト値へフォールバック。
...
- ログと観測性（漏洩防止を含む）: 追加フィールドはログ出力不要。例外時は文脈を保持した警告ロギングのみ行い、個人名（assignee）は出力しない（必要に応じてタスク ID など匿名化情報に置換）。
