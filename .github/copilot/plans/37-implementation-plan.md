...
- 機能要件:
  ...
  - Task Table に水平スクロールバーを常時表示する意図で実装し、ヘッダーとボディの水平位置がずれない。
  - Task/Schedule の水平スクロールは同期せず、分割比も永続化しない（リロードで初期値）。
...
- 変更ファイル（新規/修正/削除）:
  ...
  - 修正: `src/components/task-list/*`（Task Table のスクロールコンテナ統合、ヘッダー/ボディ整合）
  - 修正: 関連スタイルファイル（分割ハンドル、スクロール意図の明示）
  - テスト: 必要に応じて既存テストの更新または追加
- 影響範囲・互換性リスク:
  - レイアウトコンテナの構造変更によるスタイル衝突リスク。最小幅制約計算の誤りによるレイアウト破綻リスク。
  - 既存のスクロール・選択イベントと分割ハンドルイベントが干渉しないよう stopPropagation / pointer capture を検討。
- 外部依存・Secrets の扱い:
...
- 責務分離 / データフロー:
  ...
  - Task Table はヘッダー+ボディを同一水平スクロールコンテナまたは共有 scrollLeft で描画し、overflow-x: scroll を指定して常時表示の意図を担保。
...
- テスト観点（正常 / 例外 / 境界 / 回帰）:
  ...
  - Task Table 水平スクロールでヘッダーとボディ位置がずれないことの検証。
