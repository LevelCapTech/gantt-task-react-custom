...
- UI 要件（progress 列の表示内容と編集時の基本挙動）:
  ...
  - 編集確定時は入力値を `Number` 変換し、0〜100 にクランプした値を `onCellCommit` に渡す。変換不能の場合は commit を実行せず、編集 UI に `0〜100 の数値を入力してください` を表示して入力を促す（既存の OverlayEditor のエラー表示パターンに準拠）。
...
- テスト観点（正常 / 例外 / 境界 / 回帰）:
  ...
  - 正常系: 0〜100 の入力で `onCellCommit` が呼ばれ、値が step 粒度に合わせて丸められて渡される。
