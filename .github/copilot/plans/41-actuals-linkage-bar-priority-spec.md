...
- 機能要件:
  ...
  - 期間は [ActualStart, ActualEnd) の半開区間とし、ActualEffortHours は `normalized = Math.round(effort / 0.25) * 0.25` で正規化する（0.25h で割った値の小数部が 0.5 未満なら切り捨て、0.5 以上なら切り上げ）。例: 1.12→1.00、1.13→1.25。
- 非機能要件:
