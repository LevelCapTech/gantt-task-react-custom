...
- 機能要件:
  ...
  - 期間は [ActualStart, ActualEnd) の半開区間とし、ActualEffortHours は 0.25h 刻みで差分が 0.125h 未満なら下方向、0.125h 以上なら上方向へ丸める（例: 1.125→1.25）。
- 非機能要件:
...
- テスト観点（正常 / 例外 / 境界 / 回帰）:
  ...
  - 0.25h 丸め（1.12→1.00、1.13→1.25、1.37→1.25、1.38→1.50、1.125→1.25、1.375→1.50、1.625→1.75、1.875→2.00）を確認。
